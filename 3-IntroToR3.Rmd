...
Pole trójkąta obliczamy za pomocą wzoru $a*h/2$. Przepszmy zmienne:
```{r}
a <- 10
h <- 5
```

Zdefinujmy pole trójkąta:
```{r}
pole_trójkąta <- a*h/2
```
...
```{r}
dzielenie_całkowite <- 256%/%7
reszta_z_dzielenia <- 256%%7
```
...
```{r}
# Zadanie 2: Stwórz wektor z 50 losowych liczb z przedziału od 1 do 100
# ```{r}
# wektorjednsto <- c(1:100)
#````
#```{r}
#losowe <- sample(wektorjedensto, size = 50, replace=FASLE)
#```

wektor_losowy <- sample(1:100, 50, replace = TRUE)

min_wartosc <- min(wektor_losowy)
max_wartosc <- max(wektor_losowy)
suma_elementow <- sum(wektor_losowy)
...
wektor_danych <- c(3, 7, 7, 7, 2, 2, 5)

tabela_czestosci <- table(wektor_danych)
...
```{r}
# Zadanie 1: Stwórz wektor, który będzie zwracał wszystkie wartości od 0 do 1 co 0,1
...
produkty
print(produkty)
length(produkty$cena)
length(produkty$ilosc)
if (length(produkty$cena) == length(produkty$ilosc)) {
  # Dodanie kolumny 'wartość'
  produkty$wartosc <- produkty$cena * produkty$ilosc
  # Wyświetlenie zaktualizowanej ramki danych
  print(produkty)
} else {
  print("Kolumny 'cena' i 'ilość' mają różne liczby elementów.")
}
```
...
```{r}
# Sprawdzenie, czy liczba 7 jest większa od 5 i jednocześnie mniejsza od 10
wynik <- 7 > 5 & 7 < 10
wynik
```
...
```{r}
# Dla wektora [-3, 2, 5] użyj funkcji ifelse(), aby przypisać "Dodatnia" lub "Ujemna"

wektor <- c(-3, 2, 5)
etykiety <- ifelse(wektor > 0, "Dodatnia", "Ujemna")
etykiety
```
...
```{r}
# Sprawdzenie, czy jakikolwiek element wektora [FALSE, FALSE, TRUE] jest prawdziwy
...
```{r}
# Znalezienie indeksów liczb parzystych w wektorze [1, 2, 3, 4, 5, 6]

wektor <- c(1, 2, 3, 4, 5, 6)
indeksy_parzyste <- which(wektor %% 2 == 0)
indeksy_parzyste
```
...
```{r}
# Sprawdzenie, czy dana liczba jest podzielna przez 3

liczba <- 9 

if (liczba %% 3 == 0) {
  cat(liczba, "jest podzielna przez 3.\n")
} else {
  cat(liczba, "nie jest podzielna przez 3.\n")
}
```
...
```{r}
# Użycie operatora NOT, aby odwrócić wynik porównania x == y

x <- 5
y <- 10
wynik_porownania <- (x == y)
wynik_odwrocony <- !wynik_porownania

cat("Wynik porównania x == y:", wynik_porownania, "\n")
cat("Wynik po odwróceniu:", wynik_odwrocony, "\n")
```
...
```{r}
# Pętla for, która wypisuje liczby od 1 do 5
...
```{r}
# Obliczanie silni liczby 5 przy użyciu pętli while

n <- 5        # Liczba, dla której obliczamy silnię
silnia <- 1   # Inicjalizacja zmiennej do przechowywania wyniku

# Pętla while
while (n > 1) {
  silnia <- silnia * n  # Mnożenie aktualnej wartości silni przez n
  n <- n - 1             # Decrement n
}
...
```{r}
# Stworzenie macierzy 2x5 i obliczenie sumy elementów w każdym wierszu

# Tworzenie macierzy 2x5
macierz <- matrix(1:10, nrow = 2, ncol = 5, byrow = TRUE)

# Wyświetlenie macierzy
print("Macierz:")
print(macierz)

# Obliczenie sumy elementów w każdym wierszu
